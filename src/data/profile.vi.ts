...
export const hero: typeof heroEn = {
  ...
  subtext:
    'Bốn năm PHP và Laravel qua các hệ ERP, MES, y tế, kho vận và thương mại điện tử, làm từ Đà Nẵng. Service ERP tôi đang phụ trách hiện tại chạy NestJS và Vue, và mọi kết quả tối ưu trên trang này đều ra từ đó.',
}
...
export const metrics = metricsEn.map((m, i) => ({
  ...
  label: [
    ...
    'Số ca ở trên, đều nằm trong service ERP tôi phụ trách',
  ][i]!,
}))
...
export const impact: ImpactCase[] = [
  ...
  },
  {
    ...
    body: 'Một endpoint join khoảng mười tám bảng, trong đó contacts 1 triệu dòng và contact info 2 triệu. Tôi tách nó ra: bản ghi chính trả về ngay, còn dữ liệu hợp đồng nặng hơn và ít cấp bách hơn ở lần render đầu thì tải sau. Hai endpoint còn lại trên màn hình đó đi từ 6,1s xuống 187ms và từ 8,8s xuống 60ms.',
  ...
  },
  {
    ...
    before: '~10.000 ô',
    after: 'lazy-load',
    title: 'API đã nhanh rồi mà màn hình vẫn chậm',
    body: 'Một trang báo cáo vẽ một trăm dòng nhân với khoảng một trăm cột - mười hai tháng, mỗi tháng tám số liệu, cộng thêm tổng và trung bình năm - vào một vxe-table. Query đã trả về xong rồi, thứ người dùng còn phải chờ là trình duyệt. Tôi giới hạn số bản ghi frontend lấy về mỗi trang và chuyển phần cột sang lazy-load cuộn ngang, để bảng chỉ vẽ đúng phần đang hiển thị thay vì vẽ hết.',
    context: 'Màn hình báo cáo ERP, Vue 3',
  },
]
...
export const experience: Job[] = [
  {
    ...
    points: [
      'Lập trình viên duy nhất của service ERP - NestJS với TypeORM ở server, Vue 3 với TypeScript ở client, cả hai đều do tôi làm - tổng hợp dữ liệu từ B2B và MES thành báo cáo ngày và tháng.',
      'Xử lý cả năm vấn đề hiệu năng ở trên bên trong codebase ERP mà tôi kế thừa, với một cấu trúc không tốt khi nó đến tay tôi. Cái cuối cùng nằm ở frontend Vue chứ không phải ở query.',
      ...
      'Xây một tính năng AI soạn email trong ERP: nó viết email theo đúng giọng của người đang dùng, bám theo tính cách và tác phong viết của họ.',
    ],
  },
  {
    company: 'Sotatek',
    ...
    points: [
      'Team Leader của đội backend năm người trên một hệ thống quản lý bệnh viện làm từ lúc khởi động: tôi chịu trách nhiệm kiến trúc hệ thống, thiết kế database và các giải pháp kỹ thuật, đồng thời quản lý Git flow của cả đội.',
      'Dự án thay ba PM trong ba tháng và đang trôi tiến độ. Không mang chức danh đó, tôi vẫn nhảy vào gánh phần vận hành - lên kế hoạch, timeline và lịch release cho cả đội dev lẫn đội QC - và tập trung vào đúng thứ đang gây ra issue.',
      'Dựng lại quy trình release cho một dự án đang mang rủi ro bàn giao thật sự: việc release tuỳ hứng từ môi trường dev được thay bằng feature branch cắt theo sprint tách ra từ main, để QC lập kế hoạch test case được theo sprint và không có gì lên production mà chưa qua QC. Số issue không đáng có giảm xuống và các mốc release giữ được đúng hạn.',
      'Dựng server staging mà quy trình mới cần - luồng cũ không có chỗ nào để kiểm tra một bản release trước khi nó lên - và bổ sung monitoring giám sát hệ thống cùng log cho những phần cần thiết, để một issue được điều tra dựa trên bằng chứng thay vì phải đoán.',
      'Nêu rủi ro phạm vi trước khi nó kịp thành bug: khi một ticket khách muốn làm có khả năng gây vấn đề, phần đánh đổi được giải thích và xác nhận lại với khách trước khi nó vào sprint.',
      'Đưa vào checklist peer review, rồi tự xây một bước review PR bằng AI khi số lượng pull request vượt quá sức một người review. Tôi cũng kèm hai bạn developer trong giai đoạn các bạn mới vào dự án.',
  ...
  },
  {
    ...
    meta: 'Đội 16 người',
    points: [
      'Chuyển nền tảng cũ viết bằng Python và Django sang Laravel và PostgreSQL cùng một đội Scrum mười sáu người: một sản phẩm gộp cả bán hàng lẫn kho vận và tồn kho. Nó vừa bán vừa lưu kho - dữ liệu mỗi đơn bán chảy ngược về kho, và chi phí kho được tính từ đó.',
      'Đưa dữ liệu sang: import CSV khoảng 2 đến 3 triệu bản ghi mỗi lần chạy, và export ở khối lượng tương đương, viết theo kiểu insert theo lô năm mươi dòng mỗi worker thay vì tạo một model cho từng dòng. Ở khối lượng đó, khác biệt không nằm ở tốc độ mà ở chỗ job có chạy xong nổi trước khi PHP hết bộ nhớ hay không.',
      'Chỗ nào hành vi của hệ cũ không rõ ràng thì tôi đọc lại chính phần code Django gốc rồi xác nhận nghiệp vụ đúng với khách hàng thay vì đoán, để bản viết lại bám theo cách họ thật sự kinh doanh.',
      'Thiết kế schema và logic nghiệp vụ dọc toàn bộ luồng vận hành: nhập hàng vào kho, xuất hoá đơn, giao hàng khi khách đặt, tính lại số lượng tồn còn lại, và phần thống kê kinh doanh bên trên.',
      'Sửa luôn các luồng đọc phía sau, trên một màn hình khách đã phàn nàn: eager load những quan hệ mà màn hình tồn kho vốn đang truy vấn từng dòng một - một ca N+1 kinh điển - bỏ paginate() khỏi danh sách để nó thôi phải trả giá cho một câu COUNT trên toàn bảng ở mỗi request, và đánh lại index. Bảng chính khoảng một đến hai triệu dòng, join thêm nhiều bảng cùng cỡ, mà không có index nào khớp với cách màn hình thực sự lọc. Sau khi sửa thì mở ra là thấy ngay.',
      'Ôm toàn bộ phần PDF của cả hai source, tồn kho và thương mại điện tử, từ khâu sinh hoá đơn và tài liệu cho tới lúc xuất file, kèm việc gửi hoá đơn qua fax bằng API bên thứ ba.',
      'Một hàng đợi AWS SQS gánh phần đồng bộ dữ liệu theo lô toàn hệ thống. Bổ sung feature test và unit test vào quy trình backend, viết tài liệu cho module, đồng thời review code và hỗ trợ các bạn junior.',
    ],
  },
  {
    ...
    points: [
      'Full-stack trên ba nền tảng của khách hàng - phân tích da bằng AI, đặt tài xế, và quản lý dự án kèm tuyển dụng: Laravel ở backend cho cả ba, và frontend cho hai trong số đó bằng Vue 3 với TypeScript và Tailwind, tự thiết kế cấu trúc dữ liệu cho các tính năng mới.',
      ...
      'Trên nền tảng quản lý dự án và tuyển dụng (đội 18 người): theo dõi dự án, công việc và hợp đồng theo kiểu Backlog hay Asana, nối vào luồng tuyển dụng để một dự án đang thiếu người có thể tìm ứng viên - onsite, remote hoặc freelance - ngay trong hệ thống. Tích hợp Keycloak SSO, đăng nhập SNS và lưu trữ S3 hoặc FTP, đồng thời đánh lại index cho các query danh sách chậm ở màn hình dự án và công việc.',
    ],
  },
  {
    ...
    points: [
      ...
      'Sau đó chuyển sang ba nền tảng cho cùng khách hàng đó: hai hệ bán lẻ trực tuyến và một nền tảng đánh giá sản phẩm, với quản lý sản phẩm, đơn hàng và dòng hàng xuyên suốt. Stripe trên hai hệ bán lẻ: tôi làm phần Laravel và phần đẩy xử lý vào queue để endpoint trả lời ngay, làm cùng một anh senior hướng dẫn tôi phần xác thực chữ ký, xử lý chống trùng khi Stripe gửi lại event, và các luồng hoàn tiền, thất bại, đối soát.',
    ],
  },
]
...
export const featured = {
  ...
  points: [
    'Các service chia theo miền nghiệp vụ đứng sau Nginx, chứng chỉ TLS do certbot cấp: Laravel cho auth, admin, người dùng và thanh toán; Go với Gin cho cửa hàng và tồn kho; Python với FastAPI cho các tính năng AI.',
...
export const clientWork: ClientProject[] = [
  {
    ...
    body: 'Hệ ERP và MES cho doanh nghiệp, phủ kế toán, CRM, nhân sự, billing, đơn hàng, giao vận và tồn kho, kèm một tính năng AI soạn email theo đúng giọng của người đang dùng. Tôi phụ trách service ERP; các hệ B2B và MES mà nó tổng hợp dữ liệu từ đó là PHP, và tôi hỗ trợ cả chúng.',
  ...
  },
  {
    ...
    body: 'Đặt lịch khám, quản lý khoá điều trị, ca trực và hợp đồng, với OAuth qua Laravel Passport và push notification. Với vai trò Team Leader, tôi chịu trách nhiệm kiến trúc, thiết kế database và stack cho đội backend năm người ngay từ đầu dự án.',
    stack: ['Lumen', 'Laravel', 'MySQL', 'Redis', 'FCM'],
  },
  {
    name: 'Nền tảng bán hàng, kho vận và tồn kho',
    period: 'T2/2024 - T3/2025',
    client: 'Khách Nhật, đội 16 người',
    body: 'Một hệ thống cũ viết bằng Python và Django được chuyển sang Laravel và PostgreSQL, gộp bán hàng và kho vận vào một sản phẩm: mua hàng, bán hàng và quản lý cửa hàng, nhập kho, xuất hoá đơn, giao hàng theo đơn của khách, tính lại tồn kho và thống kê kinh doanh. Dữ liệu mỗi đơn bán chảy ngược về kho để tính chi phí kho. Toàn bộ phần sinh và xuất PDF của cả source này lẫn source thương mại điện tử, cộng với gửi fax qua API bên thứ ba.',
  ...
  },
  {
    ...
    body: 'Các module dự án, công việc và hợp đồng với phân quyền theo vai trò, theo kiểu Backlog hay Asana, nối vào luồng tuyển dụng để một dự án đang thiếu người có thể tìm ứng viên onsite, remote hoặc freelance ngay trong cùng hệ thống. Keycloak SSO, lưu trữ trên S3 và FTP.',
  ...
  {
    name: 'Thương mại điện tử, thanh toán và đánh giá',
    period: 'T3/2022 - T1/2023',
    client: 'Khách Nhật, 3 nền tảng',
    body: 'Một nền tảng bán kit xét nghiệm covid - quản lý sản phẩm, đơn hàng và dòng hàng, import và export CSV, thanh toán trực tuyến. Một nền tảng đánh giá sản phẩm - nơi reviewer đăng bài đánh giá, kèm kiểm duyệt, tìm kiếm và lọc, chấm điểm. Và gian hàng nhiều người bán - đăng ký người bán, đơn hàng, quản lý cửa hàng, thanh toán và giao vận. Hai trong ba nền tảng dùng Stripe, với Redis cache phần đọc danh mục.',
...
export const alsoUse: string =
  'Phía Node: TypeORM, class-validator, RabbitMQ qua @nestjs/microservices, @nestjs/schedule, Passport JWT, Vitest. Phía PHP: Lumen, Eloquent, Laravel Passport, Laravel Telescope, Laravel Reverb. Ngoài ra còn AWS S3, AWS SQS, chứng chỉ TLS bằng Let\'s Encrypt và certbot, Firebase Cloud Messaging, Stripe, Keycloak, Pusher, Google Maps API và Cloudflare Pages.'

export const runtimes: typeof runtimesEn = {
  headline: 'Hai stack, và đường nối giữa chúng',
  intro:
    'PHP và Laravel là phần sâu: bốn năm và chín hệ thống chạy thật. NestJS là thứ tôi đang phụ trách, và mọi kết quả đo được ở trên đều ra từ service tôi phụ trách. Cả hai đều là công việc hiện tại, và chỗ chúng gặp nhau là phần tôi muốn được hỏi nhất.',
  points: [
    {
      title: 'Laravel là phần sâu, và sâu bốn năm',
      body: 'Chín hệ thống chạy thật từ 2021 - nhân sự, ba nền tảng thương mại điện tử, thanh toán và đánh giá sản phẩm, đặt tài xế, quản lý dự án và tuyển dụng, phân tích da bằng AI, kho vận và tồn kho, và một nền tảng bệnh viện - trong các đội từ bảy đến hai mươi người, phần lớn cho khách Nhật. Thiết kế schema, xử lý hàng đợi, kỷ luật review và cách đọc một câu query chậm đều là những thứ tôi học trong PHP trước khi viết dòng TypeScript nào. Đây không phải nửa CV tôi đang rời bỏ; đây là nửa đã dạy tôi phần còn lại.',
    },
    {
    ...
    {
      title: 'Cả năm kết quả ở trên đều ra từ ERP tôi đang phụ trách',
      body: 'Service ERP là NestJS với TypeORM trên MySQL, cộng một datasource Postgres thứ hai cho dữ liệu đồng bộ. Bảng rollup, việc dựng lại index, việc tách endpoint và giới hạn cho batch worker đều được viết bằng TypeScript, không phải PHP, còn phần sửa bảng thì nằm ở frontend Vue của chính nó. Node không phải ngôn ngữ tôi đang mong bước vào.',
    },
    {
...
export const howIWork: typeof howIWorkEn = [
  ...
  },
  {
    ...
    body: 'Ở Sotatek tôi dẫn đội backend, và đội đang release từ một môi trường dev dùng chung, nên không có gì kiểm thử được một cách đáng tin. Chuyển sang feature branch cắt theo sprint tách từ main cho QC một thứ để lập kế hoạch, tôi dựng server staging để họ kiểm tra trên đó, và không có gì lên production mà chưa đi qua QC.',
  },
  {
...
export const contact: typeof contactEn = {
  headline: 'Sẵn sàng cho các vị trí backend senior Laravel và Node.js.',
...
export const ui: typeof uiEn = {
  ...
  impactIntro: 'Năm vấn đề trong năm vừa rồi, kèm những con số ở đầu bên kia.',
