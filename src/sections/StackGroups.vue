...
<template>
  <section class="mx-auto max-w-7xl px-5 py-20 sm:px-8 sm:py-28">
    <h2
      v-reveal="0"
      class="mt-6 max-w-[22ch] text-[1.95rem] leading-tight font-medium tracking-[-0.03em] text-ink sm:text-[2.5rem]"
    >
      {{ ui.stackHeading }}
    </h2>
    ...
    <div class="mt-11 grid gap-x-10 gap-y-10 sm:grid-cols-2 lg:grid-cols-4">
      <div
        ...
        :key="group.label"
        v-reveal="i"
        class="border-t border-line-strong pt-5"
      >
        ...
        <ul class="mt-4 space-y-3">
          <li
            ...
            :key="item.slug"
            class="flex items-center gap-2.5 text-ink"
          >
            <TechIcon :slug="item.slug" class="text-faint" />
